...
protected:
	...
	// These are selected as the first two branches of the search tree!
	bool center_is_face;
	int center_id;
	...
	Trie* constraint_trie;
	Trie* constraint_trie_backup;
	int max_constraint_trie_size;

	bool vertices_distribute_evenly;

public:
	ConstraintGenerator(Grid* grid, Configuration* conf);
